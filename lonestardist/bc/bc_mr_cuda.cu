...
__global__ void InitializeGraph(
        CSRGraph graph, 
...
__global__ void InitializeIteration(
        CSRGraph graph, 
        unsigned int __begin, 
        unsigned int __end, 
    ...
    struct CUDA_Context*  ctx)
{
  ...
  InitializeIteration <<<blocks, threads>>>(
          ctx->gg, 
          0, 
          ctx->gg.nnodes, 
...
__global__ void FindMessagetoSync(
        CSRGraph graph, 
        unsigned int __begin, 
        unsigned int __end, 
...
// TODO: WESTON: figure out what a DGA accumulator is
__global__ void ConfirmMessageToSend(
        CSRGraph graph, 
        unsigned int __begin, 
        unsigned int __end, 
        uint32_t roundNumber,
...
// TODO: WESTON: figure out what a DGA accumulator is
__global__ void SendAPSPMessages(
        CSRGraph graph, 
        unsigned int __begin, 
        unsigned int __end, 
        uint32_t local_infinity,
        ...
        uint32_t * p_roundIndexToSend)
 {
   ...
   for (index_type dst = __begin + tid; dst < dst_end; dst += nthreads)
   {
     ...
     current_edge_end = (graph).getFirstEdge((dst) + 1);
